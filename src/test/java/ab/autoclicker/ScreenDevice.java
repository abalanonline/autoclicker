...
public class ScreenDevice extends Device {
  ...
  public void update() {
    ...
    for (int i = 0; i < 8; i++) {
      int x = (s + 4) * i + 2;
      int lc = tm1638.getLed(i) ? bc[0] : 0;
      for (int iy = 1; iy <= s; iy++) for (int ix = 0; ix < s; ix++) dot(x + ix, iy, lc);
      ...
      dot(x + s + 1, y - 1, (b & 0x80) == 0 ? 0 : bc[brightness]);
